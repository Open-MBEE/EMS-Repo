...
public class MmsDiffGet extends AbstractJavaWebScript {
    ...
    protected boolean validateRequest( WebScriptRequest req, Status status ) {
        ...
        ws1 = getWorkspaceFromId( workspaceId1, services, response, status, false, null );
        ws2 = getWorkspaceFromId( workspaceId2, services, response, status, false, null );
        ...
        if ( !wsFound1 ) {
            log( LogLevel.ERROR,
                 "Bad workspaceId, " + workspaceId1 + " not found",
                 HttpServletResponse.SC_NOT_FOUND );
            return false;
        }
        if ( !wsFound2 ) {
            log( LogLevel.ERROR,
                 "Bad workspaceId, " + workspaceId2 + " not found",
                 HttpServletResponse.SC_NOT_FOUND );
            return false;
        }
        return true;
