...
public class HtmlToPdfPost extends AbstractJavaWebScript {
	...
	public EmsScriptNode convert(String docId, String tagId, String timeStamp,
			...
			String footerContent, String docNum) {
		...
		try {
			...
			String htmlPath = saveHtmlToFilesystem(htmlFilename, htmlContent,
					coverFilename, coverContent, footerFilename, footerContent, headerFilename, headerContent, tagId, timeStamp, docNum);
...
//		head.append("<link href=\"https://fonts.googleapis.com/css?family=Droid+Serif:400,700,400italic,700italic\" rel=\"stylesheet\" type=\"text/css\" />");
	...
	protected String saveHtmlToFilesystem(String htmlFilename,
			...
			String docNum) throws Throwable {
		...
		try {
			...
			createHeaderPage(headerFilename, headerContent, tagId, timeStamp, docNum);
		} catch (Throwable ex) {
...
//		Runtime rt = Runtime.getRuntime();
	...
	protected void createHeaderPage(String headerFilename,
		...
		StringBuffer html = new StringBuffer();
		if(Utils.isNullOrEmpty(headerContent)){
			Date d = new Date();
			...
			Document doc = Jsoup.parse(headerContent, "UTF-8");
			doc.body().append("<hr/>");
			doc.head().before("<style type=\"text/css\">BODY{font-size:8pt;} HR{width:100%;}</style>");
			html.append("<!DOCTYPE html>");
			html.append(doc.outerHtml());
		}
