...
//		String filePath = String.format("%s/%s.html", this.fullDocDir, viewId);
		...
		command.add(String.format("%s:%d/%s://%s/mmsFullDoc.html?ws=%s&site=%s&docId=%s&viewId=%s&section=%s&time=%s",
		preRendererUrl,preRendererPort, protocol, hostnameAndPort, workspace == null ? "master" : workspace.getName(), site, docId, viewId, section, timestamp));
