...
CreateWorkspaceDelete1
	...
	${branch_names} =	 Set Variable		test			develop			
	[Teardown]			set_wsid_to_gv1
	run curl test		370		CreateWorkspaceDelete1		Create workspace to be deleted		curl -w "\n\%{http_code}\n" -u admin:admin -X POST "http://localhost:8080/alfresco/service/workspaces/AA?sourceWorkspace=master&copyTime=$gv7"		"True"		"['"nodeRefId"', '"versionedRefId"', '"created"', '"read"', '"lastModified"', '"modified"', '"siteCharacterizationId"', 'time_total', '"parent"', '"id"', '"qualifiedId"', '"branched"']"		"['test', 'develop']"		"None"		"None"		"<function set_wsid_to_gv1 at 0x1090c5500>"		
...
ConsumerDelete
	...
	@{branch_names} =	 Set Variable		test			workspaces			develop			develop2			
	[Teardown]			removeCmNames
	run curl test		633		ConsumerDelete		Delete element with user Consumer		curl -w '\n\%{http_code}\n' -u Consumer:password -X DELETE http://localhost:8080/alfresco/service/workspaces/master/elements/y		"True"		"['"nodeRefId"', '"versionedRefId"', '"created"', '"read"', '"lastModified"', '"modified"', '"siteCharacterizationId"', 'time_total', '"timestamp"', '"id"', '"message"']"		"['test', 'workspaces', 'develop', 'develop2']"		"None"		"<function removeCmNames at 0x1090c51b8>"		"None"		
...
CheckIfPostedAspectsInHistory
	...
	@{output_filters} =	 Set Variable		"nodeRefId"		"versionedRefId"		"created"		"read"		"lastModified"		"modified"		"siteCharacterizationId"		time_total		
	@{branch_names} =	 Set Variable		test			workspaces			develop			
	run curl test		701		CheckIfPostedAspectsInHistory		Get the previously posted elements at timestamp=now to see if their type aspects were recorded properly.		curl -w "\n\%{http_code}\n" -u admin:admin -X GET "http://localhost:8080/alfresco/service/workspaces/master/elements/aspect_history_zzz?recurse=true&timestamp=$gv1"		"True"		"['"nodeRefId"', '"versionedRefId"', '"created"', '"read"', '"lastModified"', '"modified"', '"siteCharacterizationId"', 'time_total']"		"['test', 'workspaces', 'develop']"		"<function set_gv1_to_current_time at 0x1090c5f50>"		
