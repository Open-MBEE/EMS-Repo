...
if [ $server -eq 1 ]; then
	echo 'SERVER CONNECTED'
        ...
        mv master_regression_test_suite.robot master_regression_test_suite.robot.bk
        python robot_main.py
        mv regression_test_suite.robot master_regression_test_suite.robot
        cd ../../
        python test-data/javawebscripts/regression_test_harness.py -g develop
        robot test-data/javawebscripts/master_regression_test_suite.robot
  	    failTest=$?
