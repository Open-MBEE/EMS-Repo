...
echo "--------====================--------"
if [ $1 == json ]; then
    echo "python -c 'from serverUtilities import *; addViewToViewByJSON(\"$input2\",\"$input3\");'"
    python -c "from serverUtilities import *; addViewToViewByJSON('$2','$3');"

elif [ $1 == name ]; then
    echo "python -c 'from serverUtilities import *; addViewToViewByName(\"$input2\",\"$input3\");'"
